...
**Визуальный мир Героёнка (редизайн по листу персонажа), 26.09 — этап 1 выложен.** Задание владелицы: сайт как «дверь в мир Героёнка», канон — `docs/brand/character-sheet.png`, `geroenok-main.png`, `emotions-actions.png`. Персонажа не менять, не мультяшить, не перегружать им страницы.
- Картинки: `python scripts/brand_assets.py` → `assets/brand/*.webp` (бумага → белый; на сайте класс `.brand-art` = `mix-blend-mode: multiply`, работает только на светлом фоне; на тёмном — светлый медальон). Значки на кнопках — `assets/brand/star.svg`.
- `assets/geroenok.css` подключён последним на всех страницах сайта и переопределяет переменные и общие блоки встроенных стилей. Встроенные стили страниц пока остались (дублируются) — чистить постепенно.
- Сделано: палитра/шрифты (Lora, PT Sans, Caveat), шапка + меню для телефона (`<details class="nav-mobile">`), подвал с фразой, главная (первый экран — «первая страница книги»), 404.html (`not_found_handling: 404-page`).
- Дальше по плану: этап 3 — анкета как «страницы истории» + фирменные ожидание/ошибка/успех в create.html и book.html; этап 4 — остальные страницы (эмодзи в карточках → предметы мира); этап 5 — SEO (favicon, sitemap.xml, robots.txt, canonical, Open Graph), доступность, скорость, сверка с листом.
- Нет публикуемых иллюстраций с ребёнком (assets/hero, assets/kit/alex не в git — личные данные): на главной в развороте фон сцены; нужен пример с согласием родителей.
- Проверка вёрстки: playwright-core с установленным Chrome (`%TEMP%/pw/shot.mjs URL out.png mobile|desktop`); обычный headless Chrome уже 500 px не рисует честно.
